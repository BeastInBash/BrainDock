...
import { ChevronsLeft, CirclePlusIcon, MenuIcon, Plus, PlusIcon, Search, Settings, Trash } from "lucide-react"
...
export const Navigation = () => {
  ...
  const handleMouseMove = (event: MouseEvent) => {
    ...
    if (newWidth < 200) newWidth = 240;
    if (newWidth > 480) newWidth = 480;
    if (sidebarRef.current && navbarRef.current) {
      sidebarRef.current.style.width = `${newWidth}px` // setting the width of the sidebar
      navbarRef.current.style.setProperty("left", `${newWidth}px`) // Seting the position of the navbar
      navbarRef.current.style.setProperty("width", `calc(100%-${newWidth}px)`) // Setting the width of the navbar 
    }
  }
  ...
  // Resetting the width of the sidebar 
  const resetWidth = () => {
    if (sidebarRef.current && navbarRef.current) {
    ...
    <>
      <aside
        ...
        <div >
          ...
          <Item onClick={handleCreate} label="New Page" icon={CirclePlusIcon} />
        </div>
        {/* Show the created Files or notes  */}
        <div className="mt-4">
          <DocumentList />
